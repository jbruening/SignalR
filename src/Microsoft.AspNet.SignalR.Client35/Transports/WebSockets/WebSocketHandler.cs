...
namespace Microsoft.AspNet.SignalR.WebSockets
{
    public abstract class WebSocketHandler
    {
        ...
        internal Task SendAsync(string message)
        {
            ...
            return SendAsync(new ArraySegment<byte>(buffer), Opcode.Text);
        }

        internal virtual Task SendAsync(ArraySegment<byte> message, Opcode messageType, bool endOfMessage = true)
        {
            ...
            return _sendQueue.Enqueue(async state =>
            {
                ...
                try
                {
                    ...
                    if (messageType == Opcode.Binary)
                    {
                        context.Handler.WebSocket.SendAsync(context.Message.Array, completion.SetResult);
                    }
                    else if (messageType == Opcode.Text)
                    {
                        var text = Encoding.UTF8.GetString(message.Array);
                        context.Handler.WebSocket.SendAsync(text, completion.SetResult);
                    }
        ...
        public virtual Task CloseAsync()
        {
            ...
            return _sendQueue.Enqueue(async state =>
            {
                ...
                try
                {
                    await context.Handler.WebSocket.CloseTaskAsync(CloseStatusCode.Normal, "").PreserveCulture();
                }
                catch (Exception ex)
                {
                ...
                while (!disconnectToken.IsCancellationRequested && !closedReceived)
                {
                    ...
                    switch (incomingMessage.MessageType)
                    {
                        ...
                        default:
                            ...
                            await TaskEx.WhenAny(CloseAsync(), TaskEx.Delay(_closeTimeout)).PreserveCulture();
                            break;
                    }
                }
        ...
        private static bool IsClosedOrClosedSent(WebSocket webSocket)
        {
            return webSocket.ReadyState == WebSocketState.Closed ||
                   webSocket.ReadyState == WebSocketState.Closing;
        }
        ...
        private class SendContext
        {
            ...
            public SendContext(WebSocketHandler webSocketHandler, ArraySegment<byte> message, Opcode messageType, bool endOfMessage)
            {
                Handler = webSocketHandler;
                Message = message;
